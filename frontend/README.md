...
### Prerequisites
Node.js 18+, npm, MongoDB Atlas account (or local MongoDB)

### Backend
cd backend
npm install
cp .env.example .env   # fill in MONGO_URI and PORT=5000
npm run dev
...
## API endpoints
GET    /api/jobs              List all (supports ?category= ?status= ?search=)
